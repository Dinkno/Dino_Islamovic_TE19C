...
public static void main(String[] args) {
    ...
    while(true) {
        ...
        try{if(tw>0)Thread.sleep(tw);cls();}catch(Exception exc){}
    }
}
...
static {
    if (System.getProperty("os.name").contains("Windows"))
        clsFn = ()->{
            try{(p!=null?p:(p=new ProcessBuilder("cmd", "/c", "cls").inheritIO().start() )).waitFor();
            }catch(Exception e){clsExc(e);}
        };
    else
        clsFn = ()->{
            try{Runtime.getRuntime().exec("clear"); }catch(Exception e){clsExc(e);}
        };
}
static void clsExc(Exception e){
    e.printStackTrace();clsMode=false;clsFn=null;
}
static void cls(){
    if(clsMode)clsFn.run();
